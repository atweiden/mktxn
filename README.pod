...
cmdline:
  ...
  $ mktxn
  $ mktxn -m="json" path/to/transaction/journal
...
C<$name-$version-$release.txn.tar.xz> in the working directory of the
TXNBUILD.

=head2 TXNBUILD

TXNBUILD files tell C<mktxn> how to build txns in release mode with
...
Unique name of the transaction journal. Name must be valid Arch PKGBUILD
C<pkgname>. See L<pacman source|https://projects.archlinux.org/pacman.git/tree/scripts/libmakepkg/lint_pkgbuild/pkgname.sh.in>.
...
Version number of the transaction journal. Must be valid Arch PKGBUILD
C<pkgver>.
...
Description of transaction journal.
...
Transaction journal to serialize. Journal must be in working directory
of TXNBUILD.
...
SHA256 message digest of transaction journal given in C<source>.

Example value:

  "57dc9941f776a152de13da4dff556dc9794ae9aadfa0dff069f2a25e0db871e3"
...
=item L<Config::TOML|https://github.com/atweiden/config-toml>
